	...
	Message msg;
		...
		switch (msg.Receive(Client))
		{
			...
			{	
				...
				if (msg.m_Header.m_From == A_SERVER) 
				{
					WebID = pUsers->m_ID;
					cout << "Web server connect with ID: " << WebID << endl;
				}
			...
			case M_GET_ID: {
				cout << msg.m_Header.m_From << " WEB GET ID" << endl;
				if (WebID != -1)
					Message::SendMessage(Client, msg.m_Header.m_From, WebID, M_CONFIRM);
				else 
					Message::SendMessage(Client, msg.m_Header.m_From, A_BROCKER, M_NODATA);
				break;
			}
			case M_EXIT:
			{	
			...
			{	
				...
				{ 
					cout << msg.m_Header.m_From << "   Want to get message" << endl;
			...
			{	
				if (gUsers.find(msg.m_Header.m_From) != gUsers.end())
				{
					cout << "Message from " << msg.m_Header.m_From << " msg:  " << msg.m_Data << endl;
					gUsers[msg.m_Header.m_From]->time = clock(); //отметка времени
