...
def cookie():
    ...
    id = cookie.get("number")
    global ID
    if id is None:
        init();
        cookie["number"] = str(ID)
        print(cookie)
...
def exit():
    ...
    s.connect(('localhost', 1214))
    msg = MessageItem()
    SendMessage(s, Addresses.A_BROCKER, 0, Types.M_EXIT, "py_server")
    if ReceiveData(s, msg)==Types.M_INIT:
        global ID
        ID = msg.to
        print(ID)
...
def check_NewMessage():
    ...
    msg = MessageItem()
    SendMessage(s, Addresses.A_BROCKER, int(ID), Types.M_GETDATA)
...
class Message:
    ...
    def DeleteItem(self):
        ...
        s.connect(('localhost', 1214))
        cookie();
        SendMessage(s, int(ID), 10, Types.M_DELETE, str(id))
        print(str(id))
        s.close()
